# Paramount Care — Mobile (M5: first real onboarding form)

Expo + React Native + TypeScript + Expo Router. M3 built the architecture/navigation/auth foundation; M4 wired the first complete applicant journey against the real backend (invitation → account → verify → sign in → My Onboarding dashboard → create-or-resume a session). **M5 migrates the first real onboarding form — Personal Information — and establishes the reusable save/validation/conflict pattern later step forms will follow.**

See `docs/ARCHITECTURE_DECISION_RECORDS.md` ADR-017 (M3), ADR-018 (M4), and ADR-019 (M5) for the durable record of the decisions summarized here. See `mobile/QA_WALKTHROUGH.md` for the manual test script.
...
- **`[stepId].tsx`** — a small registry (`REAL_STEP_SCREENS: Record<string, ComponentType>`) maps a migrated step id to its real screen component; every other step id still falls through to the "available in an upcoming mobile release" placeholder. Migrating the next step (M6+) means adding one entry to this map, not restructuring how any step is reached.
...
No screen-rendering snapshot tests — deliberately, per the instruction to avoid brittle snapshot-heavy tests, and consistent with the M3/M4 precedent. Screens (`home.tsx`, `onboarding/*`, `PersonalInfoScreen.tsx`) are thin renderers over `useSession()`/`useAuth()`/`usePersonalInfoForm()`, all independently tested; the wiring between `(app)/_layout.tsx`'s auth guard, `SessionProvider`'s mount timing, and `[stepId].tsx`'s real-vs-placeholder registry is verified by direct code inspection and the manual QA walkthrough (`QA_WALKTHROUGH.md`), not an automated full-navigator render test.
