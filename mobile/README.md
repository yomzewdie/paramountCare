# Paramount Care — Mobile (M6: second real onboarding form)

Expo + React Native + TypeScript + Expo Router. M3 built the architecture/navigation/auth foundation; M4 wired the first complete applicant journey against the real backend (invitation → account → verify → sign in → My Onboarding dashboard → create-or-resume a session); M5 migrated the first real onboarding form (Personal Information) and established the reusable save/validation/conflict pattern. **M6 migrates a second real form — Employment Reference #1 — specifically to test whether that pattern actually generalizes, and records what did and didn't.**

See `docs/ARCHITECTURE_DECISION_RECORDS.md` ADR-017 (M3), ADR-018 (M4), ADR-019 (M5), and ADR-020 (M6) for the durable record of the decisions summarized here. See `mobile/QA_WALKTHROUGH.md` for the manual test script.
...
| Employment Application rejected for M6 | Its packet config (`requiresSignature: true`) and data (felony-conviction disclosure, license discipline/revocation/investigation status, a state-issued license number) were identified during pre-flight and never implemented — chose Employment Reference instead. See ADR-020. |
