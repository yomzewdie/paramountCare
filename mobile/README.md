# Paramount Care — Mobile (M7: real applicant sequence, Employment Application)

Expo + React Native + TypeScript + Expo Router. M3 built the architecture/navigation/auth foundation; M4 wired the first complete applicant journey against the real backend; M5 migrated the first real onboarding form (Personal Information) and established the reusable save/validation/conflict pattern; M6 migrated a second form (Employment Reference #1) to confirm that pattern generalizes. **From M7 forward, step selection is no longer risk-ordered — it strictly follows the real Paramount packet sequence. The earliest step still missing a mobile implementation was Employment Application (not Employment Reference #2 or anything easier), so M7 implements it even though it's the largest form built so far.**

See `docs/ARCHITECTURE_DECISION_RECORDS.md` ADR-017 (M3), ADR-018 (M4), ADR-019 (M5), and ADR-020 (M6) for the durable record of the decisions summarized here — M7 introduced no new durable architectural decision, so it has no new ADR (see "Third real form" below for why). See `mobile/QA_WALKTHROUGH.md` for the manual test script.
